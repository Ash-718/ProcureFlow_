...
logger = logging.getLogger("innovategov.ai.embeddings")

# `lru_cache` memoises the *result* but does not serialise the call, so two
# threads arriving together would both construct a SentenceTransformer and load
# the model twice. That was impossible in the standalone service, where the
# provider was built once on the startup thread; in-process, the background
# warm-up and a request handler genuinely can race.
...
def get_embedding_provider() -> EmbeddingProvider:
    ...
    Falls back to the local model whenever the LLM path is selected without an
    API key — it logs and degrades rather than raising, so an offline demo
