...
logger = logging.getLogger("innovategov.ai.text_generation")

#: See the note in `embeddings.py` — same construction race, same guard.
...
def get_text_generation_provider() -> TextGenerationProvider:
    with _provider_lock:
        return _build_text_generation_provider()


def _build_text_generation_provider() -> TextGenerationProvider:
